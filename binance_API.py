...
from telegram import chat_id, bot_token
import requests
import numpy as np
import pandas as pd
...
symbol = 'ETHUSDT'  # trading coin
client = Client(api_key, api_secret)

maxposition = 0.01  # maximum lot size
stop_percent = 0.01  # Stop loss
closing_profitable_positions = [[20, 1], [40, 1], [60, 2], [80, 2], [100, 2], [150, 1], [200, 1], [200, 0]]
proffit_array = copy.copy(closing_profitable_positions)
...
def indicator_average_true_range(source_DF, n):
    df = source_DF.copy()
...
def local_minimum_indicator(DF, i):
    df = DF.copy()
    LCC = 0
...
def local_maximum_indicator(DF, i):
    df = DF.copy()
    HCC = 0
    if df['close'][i] >= df['close'][i + 1] and df['close'][i] >= df['close'][i - 1] and df['close'][i + 1] < \
            df['close'][i - 1]:
...
def prepare_data_frame(DF):
    ...
    df = indicator_average_true_range(ohlc, 14).reset_index()
    df['slope'] = indicator_slope(df['close'], 5)
...
def check_if_signal(symbol):
    ...
    if local_maximum_indicator(prepared_df, i - 1) > 0:
...
def telegram_bot_sendtext(bot_message):
    ...
    bot_chatID = chat_id
    send_text = 'https://api.telegram.org/bot' + bot_token2 + '/sendMessage?chat_id=' + bot_chatID + \
